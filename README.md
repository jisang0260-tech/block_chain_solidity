...
현재 구현 기준으로 `purchaseImage()`는 결제를 기존 등록자에게 전달하지만, 이미지의 소유권을 새로운 구매자로 이전하지는 않습니다. README 아래 설명도 이 실제 동작 기준으로 정리되어 있습니다.
...
- 이벤트와 커스텀 에러를 사용해 상태 변화와 실패 원인 추적
...
사용 목적:
- 판매 정책이 바뀌어 가격을 조정할 때
- 프론트엔드나 백오피스에서 판매가를 다시 반영할 때
...
중요한 점:
- 현재 구현에서는 구매 후에도 `imageOwners[pHash]` 값이 바뀌지 않습니다.
- 즉, 이 함수는 "결제 처리" 역할이며 "소유권 이전"까지 수행하지 않습니다.
...
현재는 `pHash`만 저장하지만, 이후 메타데이터 필드를 확장할 때 재사용하기 좋은 형태입니다.

### `getOwner(string pHash)`

이미지의 현재 등록 소유자 주소를 반환합니다.

### `getPrice(string pHash)`

이미지의 현재 판매 가격을 반환합니다.
...
컨트랙트는 `require(string)` 대신 커스텀 에러를 사용해 가스 효율과 디버깅 가독성을 높였습니다.
...
- `purchaseImage()`에는 `nonReentrant`가 적용되어 있어 재진입 공격을 방지합니다.
- 결제는 low-level `call`로 처리되며, 송금 실패 시 전체 트랜잭션이 revert 됩니다.
- `isRegistered()`는 내부적으로 저장된 `pHash` 문자열 길이를 기준으로 등록 여부를 판단합니다.
...
`hardhat.config.js`는 위 두 값이 모두 있을 때 `sepolia` 네트워크를 활성화합니다.

배포 예시:

```bash
npx hardhat ignition deploy ./ignition/modules/ImageAuthenticator.js --network sepolia
```
...
아래 스크립트는 배포 후 등록, 조회, 가격 변경, 구매 흐름을 한 번에 확인합니다.

```bash
npx hardhat run scripts/check-image-authenticator.js
```
...
동작 방식:
- 이미 등록된 `pHash`면 등록 대신 기존 소유자와 가격을 출력
- 등록되지 않은 `pHash`면 새로 등록 후 결과 출력

## Test Coverage Summary

현재 테스트는 아래 핵심 흐름을 검증합니다.
...
- 구매 시 실제 소유권 이전 기능 추가
- 이미지 메타데이터 URI 또는 원본 해시 추가 저장
- 이미지 목록 조회 기능 추가
- 관리자 기능 또는 수수료 정책 추가
- 구매 이력 저장 기능 추가
